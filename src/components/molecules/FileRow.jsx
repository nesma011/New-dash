...
function FileTypeBadge({ type }) {
  return (
    <div className="flex h-9 w-9 flex-col items-center justify-center rounded-xl bg-slate-100 text-[9px] font-semibold uppercase leading-none text-slate-500 dark:bg-slate-800 dark:text-slate-300">
...
function FileRow({ file, isLoading = false }) {
  if (isLoading) {
    return (
      <div className="flex items-center gap-3 rounded-2xl px-1 py-2">
        <div className="h-9 w-9 rounded-xl bg-slate-100 dark:bg-slate-800" aria-hidden="true" />
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl px-1 py-2">
      ...
      {file.downloadable ? (
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">
          v
        </div>
      ) : null}
    </div>
